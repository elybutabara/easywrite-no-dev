  ...
  await knex.schema.hasColumn('book_scenes', 'content').then(async (exists) => {
    if (!exists) {
      await knex.schema.table('book_scenes', function (table) {
        ...
        await knex.raw('UPDATE book_scenes SET content = ( SELECT BSV.content FROM book_scene_versions AS BSV WHERE BSV.book_scene_id = book_scenes.uuid ORDER BY BSV.is_current_version DESC, BSV.created_at DESC LIMIT 1)')
