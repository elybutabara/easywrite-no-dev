    ...
    <div ref="message-center">
        <div class="card shadow mb-4">
            <div class="card-header py-3 inline">
                <b-form inline>
                    <div class="col-md-6"><h6 class="m-0 font-weight-bold">{{ trans("site.message-center") }}</h6></div>
                    <div class="col-md-6">
                        ...
                        <button v-on:click.prevent="openMessage(null, $event)" class="btn btn-default" style="height: 24px; line-height: 24px; text-transform: none;float:right;">{{ trans("site.new-message") }}</button>
                    </div>
                </b-form>
            </div>
                            ...
                            <div class="col-md-10">
                                ...
                                <div class="" style="margin-left: 10px;">
                                    ...
                                    <div class="mt-2" style="font-size: 14px; line-height: 100%;">
                                        {{model.alias +' '+ trans('site.has-invited-you-to-read') +' '+ model.book.title  }}
                                    </div>
                                    <!--ACTION-->
                                    <div class="mt-2">
                                        <a class="btn btn-dark btn-sm" href="javascript:void(0)" @click="acceptInvite(model)">{{ trans('site.sendmail.accept') }}</a>
                                        <a @click="declineInvite(model)" class="btn btn-dark-red btn-sm">{{ trans('site.sendmail.decline') }}</a>
                                        <a class="btn btn-secondary btn-sm" href="javascript:void(0)"
                                           @click="showBookDetailsModal(model.book)">
                                            <i class="fas fa-book fa-fw"></i> {{ trans('site.book-directory-preview') }}
                                        </a>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-10">
                                <div style="margin-left: 10px;">
                                    <div>
                                        <div style="font-size: 12px; font-weight: bold; line-height: 100%;">{{model.alias}}</div>
                                        <div class="mt-2" style="font-size: 14px; line-height: 100%;">
                                            {{ model.alias +' '+trans('site.has-queried-you-to-request-an-access-to-read')+' '+ model.book.title+'.'}}
                                            <a href='javascript:void(0)' @click="CHANGE_COMPONENT({
                                                ...
                                                {{ capitalizeFirstLetter(trans('site.click-here')) }}
                                            </a> {{ trans('site.to-view') }}
                                        </div>
                                        <!--                    <div class="mt-2" style="font-size: 14px; line-height: 100%;">-->
                            ...
                            <div class="col-md-10">
                                <div style="margin-left: 10px;">
                                    <div><div style="font-size: 12px; font-weight: bold; line-height: 100%;">{{ model.alias }}</div>
                                        <div class="mt-2" style="font-size: 14px; line-height: 100%;">
                                            {{ model.alias +' '+trans('site.commented-on-chapter')+' '+ model.chapter.title+'.'}}
                            ...
                            <div class="col-md-10">
                                <div style="margin-left: 10px;">
                                    <div>
                                        <div style="font-size: 12px; font-weight: bold; line-height: 100%;">{{ model.alias }}</div>
                                        <div class="mt-2" style="font-size: 14px; line-height: 100%;">
                                            {{ model.alias +' '+trans('site.commented-on-scene')+' '+ model.scene.title+'.'}}
                                ...
                                <div class="" style="margin-left: 10px;">
                                    <div>
                                        ...
                                        <div v-if="model.action=='accept'" class="mt-2" style="font-size: 14px; line-height: 100%;">
                                            {{ model.alias +' '+trans('site.has-accepted-your-invitation-to-read')+' '+ model.book.title+'.'}}
                                        </div>
                                        <!--declined-->
                                        <div v-if="model.action=='declined'" class="mt-2" style="font-size: 14px; line-height: 100%;">
                                            {{ model.alias +' '+trans('site.has-declined-your-invitation-to-read')+' '+ model.book.title+'.'}}
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <div v-if="model.action=='post'" class="mt-2" style="font-size: 14px; line-height: 100%;">
                                            ...
                                            <span v-if="model.parent_id && model.parent_name == 'scene' ">
                                                {{ model.alias +' '+trans('site.has-left-a-feedback-on')+' '+ model.book.title +'-'+model.chapter.title+' ('+model.scene.title+' Scene).'}}
                                            ...
                                            <span v-else-if="model.chapter_id">
                                                {{ model.alias +' '+trans('site.has-left-a-feedback-on')+' '+ model.book.title +'-'+model.chapter.title+'.'}}
                                            ...
                                            <!--BOOK FEEDBACK-->
                                            <span v-else>
                                                {{ model.alias +' '+trans('site.has-left-a-feedback-on')+' '+ model.book.title+'.'}}
                                        ...
                                        <div v-if="model.action=='response'" class="mt-2" style="font-size: 14px; line-height: 100%;">
                                            <!--SCENE FEEDBACK-->
                                            <span v-if="model.parent_id && model.parent_name == 'scene' ">
                                                {{ model.alias +' '+trans('site.has-replied-to-your-feedback')+' '+ model.book.title +'-'+model.chapter.title+' ('+model.scene.title+' Scene).'}}
                                            ...
                                            <!--CHAPTER FEEDBACK-->
                                            <span v-else-if="model.chapter_id">
                                                {{ model.alias +' '+trans('site.has-replied-to-your-feedback')+' '+ model.book.title +'-'+model.chapter.title+'.'}}
                                            ...
                                            <!--BOOK FEEDBACK-->
                                            <span v-else>
                                                {{ model.alias +' '+trans('site.has-replied-to-your-feedback')+' '+ model.book.title+'.'}}
                                        ...
                                        <div v-if="model.action=='inlined' " class="mt-2" style="font-size: 14px; line-height: 100%;">
                                            {{ model.alias +' '+trans('site.commented-on')+' '+ model.book.title +'-'+model.chapter.title+'.'}}
                                            <a href='javascript:void(0)' @click="openChapterDetails(model,'open-feedback')">
                                                {{ capitalizeFirstLetter(trans('site.click-here')) }}
                                            </a> {{ trans('site.to-view') }}
                                        </div>
                            ...
                            <div class="col-md-10">
                                <div style="margin-left: 10px;">
                                    <div>
                                        <div style="font-size: 12px; font-weight: bold; line-height: 100%;">{{model.alias}}</div>
                                        <div class="mt-2" style="font-size: 14px; line-height: 100%;">
                                            {{ model.alias +' '+trans('site.sent-you-a-message')+'.'}}
                                    ...
                                    <div>
                                        ...
                                        <div class="mt-2" style="font-size: 14px; line-height: 100%;">
                                            <span v-if="model.action == 'query'">
                                                {{ model.alias +' '+trans('site.has-queried-you-to-read')+' '+ model.book.title+'.'}}
                                                <a href='javascript:void(0)' @click="CHANGE_COMPONENT({
                                            ...
                                            <span v-if="model.action == 'notify'">
                                                {{ model.alias +' '+ trans('site.sent-you-a-notification') +'.'}}
                                            ...
                                            <span v-else-if="model.action == 'reader-decision'">
                                                ...
                                                <div v-else-if="model.decision=='accept-later'" class="mt-2" style="font-size: 14px; line-height: 100%;">
                                                    {{ model.alias +' '+trans('site.has-declined-your-invitation-to-read-but-accept-later')+' '+ model.book.title+'.'}}
                                                    <span v-if="model.letter" class="'ellipsis-2'">{{ model.letter }}</span>
                                                </div>
                                                <div v-else class="mt-2" style="font-size: 14px; line-height: 100%;">
                                                    {{ model.alias +' '+trans('site.has-not-yet-decided')+' '+ model.book.title+'.'}}
                                                    <span v-if="model.letter" class="'ellipsis-2'">{{ model.letter }}</span>
                                                </div>
                                            </span>
                                        </div>
                            ...
                            <div class="col-md-10">
                                <div style="margin-left: 10px;">
                                    <div>
                                        <div style="font-size: 12px; font-weight: bold; line-height: 100%;">{{model.alias}}</div>
                                        <div class="mt-2" style="font-size: 14px; line-height: 100%;">
                                            <span v-if="model.query_decision === 1">{{ model.alias +' '+trans('site.has-accepted-your-query-for-requesting-to-read')+' '+ model.book.title+'.'}}
                                            </span>
                                            <span v-else-if="model.query_decision === 2">{{ model.alias +' '+ trans('site.has-declined-your-query-for-requesting-to-read') +' '+ model.book.title+'.'}}
                                            </span>
...
<script>
import Vue from 'vue'
import FeedbackAndBookDetailsModal from '../components-v2/FeedbackAndBookDetailsModal'
...
export default {
  ...
  components: {
    'feedback-and-book-details-modal': FeedbackAndBookDetailsModal,
    'messaging-popup': messagingPopup
  },
  computed: {},
  methods: {
  ...
  created () {
    ...
    window.addEventListener('click', function (e) {
      /*
            ...
            * */
      if (scope.$parent.notification.show && document.getElementById('message-center').contains(e.target) == false) {
        scope.$parent.notification.show = false
      }

      /*
            * Trigger <a> tag ang button as close MessageCenter
            * */
      document.querySelectorAll('#message-center a:not(.actionLink) , #message-center button, #right-menu-actions a:not(.actionLink)').forEach(link => {
        link.onclick = () => {
          scope.$parent.notification.show = false
        }
      })
