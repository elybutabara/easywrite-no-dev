            ...
            <div class="es-col fadeIn animated" v-for="note in notes" v-bind:key="note.id">
                <div class="es-card">
                    ...
                    <div class="es-card-footer">
                        ...
                        <button v-if="note.parent === 'book'" class="btn-"  @click="CHANGE_COMPONENT({tabKey: 'book-details-' + note.book.uuid, tabComponent: 'books-i-read-book-details', tabData: { book: note.book, author: note.book.author, genre: note.book.genre, about: note.book.about, uuid: note.parent_id, title: note.book.title}, tabTitle:  $t('VIEW')+ ' - ' + note.book.title, newTab: true})"><i class="lar la-eye"></i> {{$t('VIEW').toUpperCase()}}</button>
