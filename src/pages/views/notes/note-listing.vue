            ...
            <div class="es-col fadeIn animated" v-for="note in notes" v-bind:key="note.id">
                <div class="es-card">
                    ...
                    <div class="es-card-footer">
                        ...
                        <button v-if="note.parent === 'chapter'" class="btn-"  @click="CHANGE_COMPONENT({tabKey: 'chapter-details-' + note.chapter.uuid, tabComponent: 'chapter-details', tabData: { book: note.book, chapter: note.chapter }, tabTitle:  $t('VIEW')+ ' - ' + note.chapter.title, newTab: true})"><i class="lar la-eye"></i> {{$t('VIEW').toUpperCase()}}</button>
                        <button v-if="note.parent === 'scene'" class="btn-"  @click="CHANGE_COMPONENT({tabKey: 'scene-details-' + note.scene.uuid, tabComponent: 'scene-details', tabData: { book: note.book, scene: note.scene, chapter: null }, tabTitle:  $t('VIEW')+ ' - ' + note.scene.title, newTab: true})"><i class="lar la-eye"></i> {{$t('VIEW').toUpperCase()}}</button>
                        <button v-if="note.parent === 'book'" class="btn-"  @click="CHANGE_COMPONENT({tabKey: 'book-details-' + note.book.uuid, tabComponent: 'book-details', tabData: { book: note.book }, tabTitle:  $t('VIEW')+ ' - ' + note.book.title, newTab: true})"><i class="lar la-eye"></i> {{$t('VIEW').toUpperCase()}}</button>
